...
try:
    ...
    # ==============================
    # EMAIL SETTINGS - SAFE WITH FALLBACK
    # ==============================
    # Always use console backend initially to prevent startup crashes
    ...
    EMAIL_TIMEOUT = 5  # Very short timeout
    EMAIL_USE_SSL = False

    # Try to setup SendGrid only after app is running
    SENDGRID_API_KEY = os.environ.get("SENDGRID_API_KEY")
    if not DEBUG and SENDGRID_API_KEY:
        print("🚀 ATTEMPTING SENDGRID SETUP...")
        try:
            ...
            if result == 0:
                ...
                print("✅ SENDGRID: Successfully configured")
            else:
                print("❌ SENDGRID: Cannot reach SendGrid servers")
    ...
    print(f"📧 EMAIL_BACKEND: {EMAIL_BACKEND.split('.')[-1]}")
    ...
    print(f"💥 CRITICAL ERROR IN SETTINGS: {critical_error}")
    print("🔄 USING EMERGENCY FALLBACK SETTINGS")
    
    # Emergency minimal settings
    BASE_DIR = Path(__file__).resolve().parent.parent
    SECRET_KEY = "emergency-fallback-key-2024"
    DEBUG = True
    ALLOWED_HOSTS = ["*"]
    DATABASES = {
        'default': {
    ...
    print("✅ EMERGENCY SETTINGS LOADED - APP SHOULD START")